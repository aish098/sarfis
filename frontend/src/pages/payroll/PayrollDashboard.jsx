...
export default function PayrollDashboard({ onNavigateToTab }) {
  const kpis = [
    ...
    { label: 'Journal Status', value: 'READY TO POST', change: 'Period close: Aug 2026', icon: BookOpen, color: 'text-amber-600 bg-amber-50' },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-slate-900 to-indigo-950 text-white rounded-3xl p-6 shadow-xl relative overflow-hidden border border-slate-800">
        ...
        <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="bg-slate-800/40 backdrop-blur-md p-4 rounded-2xl border border-slate-700/50 min-w-[240px] space-y-3">
            ...
            <p className="text-[10px] text-slate-400 font-medium">Calculated 212 of 245 employee sheets</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-4 bg-white p-5 rounded-3xl border border-slate-100 shadow-xs text-center font-semibold text-slate-600">
        <div>
        ...
        <div className="border-l border-slate-100">
          <p className="text-[9px] text-slate-400 uppercase font-extrabold tracking-wider">Missing Structure</p>
          <p className="font-display font-black text-2xl text-rose-600 mt-1">{headcount.missingStructure}</p>
        </div>
      </div>
          ...
          <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-xs space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <button 
                ...
                className="p-3 border border-slate-200 hover:bg-slate-50 text-slate-700 rounded-2xl flex flex-col items-start gap-2.5 transition-all text-xs font-black text-left shadow-2xs group"
              >
              ...
              {warnings.map(warn => (
                <div key={warn.id} className="p-3.5 bg-slate-50/50 hover:bg-slate-50 border border-slate-200 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 transition-all">
                  ...
                  <button 
                    ...
                    className="px-3 py-1.5 bg-white border border-slate-200 hover:border-slate-300 rounded-xl text-[10px] font-black text-slate-700 transition-all shadow-3xs flex items-center gap-1 self-end sm:self-center"
                  >
