...
export default function PayrollPage() {
  ...
  return (
    <div className="p-4 lg:p-7 pb-20 max-w-[1600px] mx-auto font-sans relative overflow-hidden bg-gradient-to-br from-[#FAFBFD] via-[#FAF9F8] to-[#F5FAF8] space-y-6">
      ...
      {/* Ledger-Themed Unified Header Container */}
      <div className="w-full bg-white rounded-2xl p-5 shadow-sm border border-slate-100 relative overflow-hidden space-y-5">
        {/* Top Header Row: Branding, Status, Search, Controls & Primary Action */}
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-5">
          
          {/* Left: Branding & Subtitle */}
          <div className="flex items-start md:items-center gap-3.5">
            <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-[#10b981] to-[#06b6d4] flex items-center justify-center text-white shadow-md shadow-emerald-500/10 shrink-0">
              <Users size={20} className="stroke-[2.5]" />
            </div>
            <div>
              <div className="flex items-center gap-2.5 flex-wrap">
                <h1 className="font-display font-extrabold text-xl md:text-2xl text-[#064E3B] tracking-tight uppercase">
                  Payroll Workspace
                </h1>
                <div className="flex items-center gap-1.5 bg-emerald-50 text-emerald-800 border border-emerald-200/60 px-3 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider">
                  <span className="w-2 h-2 rounded-full bg-[#10b981] animate-pulse" />
                  Operational Lifecycle
                </div>
              </div>
              <p className="text-slate-500 text-xs font-semibold mt-0.5 max-w-xl">
          ...
          <div className="flex flex-wrap items-center gap-3 relative z-20">
            ...
            <button 
              ...
              className="flex items-center gap-2 bg-white border-2 border-slate-100 hover:border-emerald-500/40 px-3.5 py-2 rounded-xl text-slate-600 hover:text-slate-900 transition-all text-xs font-bold shadow-2xs cursor-pointer"
            >
              <Search size={14} className="text-[#10b981]" />
              <span>Search</span>
              <kbd className="hidden sm:inline-block bg-slate-100 text-slate-500 border border-slate-200 px-1.5 py-0.5 rounded text-[9px] font-mono font-bold">Ctrl+F</kbd>
            </button>
            ...
            <div className="relative">
              <button 
                ...
                className="p-2.5 bg-white border-2 border-slate-100 hover:border-emerald-500/40 rounded-xl text-slate-600 hover:text-slate-900 transition-all cursor-pointer relative shadow-2xs"
              >
                <Bell size={16} className="text-[#10b981]" />
                {notifications.filter(n => !n.read).length > 0 && (
                  <span className="absolute -top-1 -right-1 bg-rose-500 text-white font-mono text-[9px] font-black w-4 h-4 rounded-full flex items-center justify-center ring-2 ring-white">
              ...
              {showNotifications && (
                <div className="absolute right-0 mt-3 w-80 bg-white border border-slate-200 rounded-2xl shadow-2xl p-4 z-50 space-y-3 text-xs font-semibold animate-in fade-in zoom-in-95 duration-150">
                  <div className="flex justify-between items-center border-b border-slate-100 pb-2.5">
                    <span className="font-black text-slate-800 text-xs uppercase tracking-wider flex items-center gap-2">
                      <Bell size={13} className="text-[#10b981]" /> Notification Center
                    </span>
                    <button onClick={() => setShowNotifications(false)} className="text-slate-400 hover:text-slate-700"><X size={14} /></button>
                  </div>
                  <div className="space-y-2.5 max-h-64 overflow-y-auto custom-scrollbar pr-1">
                    {notifications.map(n => (
                      <div key={n.id} className="flex gap-2.5 items-start leading-snug p-2 rounded-lg hover:bg-slate-50 transition-colors">
                        <span className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${n.read ? 'bg-slate-300' : 'bg-[#10b981]'}`} />
                        <div>
                          <p className={`${n.read ? 'text-slate-400 font-normal' : 'text-slate-800 font-bold'}`}>{n.text}</p>
            ...
            <div className="flex items-center gap-2 bg-white border-2 border-slate-100 px-3 py-2 rounded-xl text-xs shadow-2xs">
              <UserCheck size={14} className="text-[#10b981] shrink-0" />
              <select
                value={userRole}
                onChange={e => setUserRole(e.target.value)}
                className="text-[11px] font-bold uppercase text-slate-800 bg-transparent border-none outline-none cursor-pointer focus:ring-0"
              >
                <option value="HR Officer" className="bg-white text-slate-800">HR Officer</option>
                <option value="HR Manager" className="bg-white text-slate-800">HR Manager (Admin)</option>
                <option value="Finance" className="bg-white text-slate-800">Finance Director</option>
                <option value="Treasury" className="bg-white text-slate-800">Treasury Officer</option>
                <option value="Auditor" className="bg-white text-slate-800">Auditor (Read Only)</option>
              </select>
            </div>

            {/* Primary Action Button (Matching Ledger PDF Button Style) */}
            <button
              onClick={() => handleNavigateToTab('processing')}
              className="flex items-center gap-2 bg-[#10b981] hover:bg-[#059669] text-white font-extrabold px-4 py-2 rounded-xl text-xs uppercase tracking-wider shadow-md transition-all cursor-pointer active:scale-95 border-none outline-none"
            >
        ...
        <div className="relative z-10 pt-3 border-t border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-1.5 bg-slate-100/70 p-1.5 rounded-xl border border-slate-200/60 overflow-x-auto custom-scrollbar whitespace-nowrap">
            {tabs.map(tab => {
              const Icon = tab.icon;
              ...
              return (
                <button
                  ...
                  className={`px-3.5 py-1.5 rounded-lg transition-all uppercase tracking-wider text-[11px] font-extrabold flex items-center gap-2 cursor-pointer ${
                    isActive 
          ...
          <div className="hidden xl:flex items-center gap-4 text-[10px] font-mono text-slate-500 uppercase tracking-wider bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-200/60">
            <span className="text-slate-400 font-bold">Shortcuts:</span>
            <span className="flex items-center gap-1"><kbd className="bg-white px-1.5 py-0.5 rounded border border-slate-200 text-slate-700 font-bold">Ctrl+F</kbd> Search</span>
            <span className="flex items-center gap-1"><kbd className="bg-white px-1.5 py-0.5 rounded border border-slate-200 text-slate-700 font-bold">Ctrl+G</kbd> Engine Wizard</span>
            <span className="flex items-center gap-1"><kbd className="bg-white px-1.5 py-0.5 rounded border border-slate-200 text-slate-700 font-bold">Ctrl+R</kbd> Reports</span>
          </div>
        </div>
      </div>
