/**
 * LeadershipPage.jsx  —  SCAFIS Premium Leadership (Polished Flow)
 *
 * Sections Ordered:
 *  1. Hero
 *  2. Hierarchy Intro (Organisation label)
 *  3. Founder Spotlight (RZ)
 *  4. Advisors Section (Mentors)
 *  5. CEO Spotlight (AK)
 *  6. Team Section (FA, HT, SM cards)
 *  7. Team Marquee (Finale)
 */
    ...
    <section style={{background:C.bg,padding:"96px 24px 0"}}>
      <div style={{maxWidth:1080,margin:"0 auto"}}>
        <FV>
          ...
          <SectionTitle>Leadership Hierarchy</SectionTitle>
          <p style={{fontFamily:"var(--font-sans)",fontSize:14,color:C.textSec,
            margin:"10px 0 0",maxWidth:460,lineHeight:1.7}}>
...
function FounderSpotlight() {
  return (
    <section style={{background:C.bg,padding:"64px 24px 72px"}}>
      <div style={{maxWidth:1080,margin:"0 auto"}}>
...
/* ── Advisors Section ──────────────────────────────────────────────────────── */
const ADVISORS = [
  { initials: "MS", name: "Prof. Mohammad Saad Anwar", title: "Taxation Lawyer", accent: "#22d3ee",
    glow: "rgba(34,211,238,0.12)", expertise: ["Tax Law", "Analytics", "Regulatory"],
  ...
  { initials: "MR", name: "Prof. Muhammed Rehan Anjum", title: "Accounting Specialist", accent: "#f59e0b",
    glow: "rgba(245,158,11,0.12)", expertise: ["IFRS", "Compliance", "Data Integrity"],
    desc: "Leads advisory on accounting standards and regulatory compliance, ensuring audit-grade accuracy across all SCAFIS modules.",
...
function AdvisorCard({ adv, idx }) {
  ...
  return (
    <M.div initial="hidden" whileInView="show" viewport={{once:true,amount:.1}} variants={idx===0?left:right_v}
      onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{flex:"1 1 320px",minWidth:300,background:hov?`linear-gradient(145deg,#13182e,${C.cardDeep})`:C.card,
        border:`1px solid ${hov?adv.accent+"55":C.border}`,borderRadius:20,overflow:"hidden",cursor:"default",
        transition:"all .32s cubic-bezier(.22,1,.36,1)",boxShadow:hov?`0 20px 60px ${adv.accent}22, 0 0 0 1px ${adv.accent}18`:"none"}}>
      <div style={{height:3,background:hov?`linear-gradient(90deg,${adv.accent},${adv.accent}88,transparent)`:`linear-gradient(90deg,${adv.accent}44,transparent)`,transition:"all .32s"}}/>
      <div style={{display:"flex",gap:20,alignItems:"flex-start",padding:"28px 28px 0",position:"relative"}}>
        <div style={{position:"absolute",top:-20,right:-20,width:140,height:140,borderRadius:"50%",background:hov?adv.glow:"transparent",filter:"blur(32px)",transition:"all .4s",pointerEvents:"none"}}/>
        <div style={{position:"relative"}}><Avatar initials={adv.initials} size={80} accent={adv.accent}/>
          <div style={{position:"absolute",bottom:2,right:2,width:14,height:14,borderRadius:"50%",background:adv.accent,border:`2px solid ${C.card}`,boxShadow:`0 0 8px ${adv.accent}`}}/>
        </div>
        <div style={{flex:1,paddingTop:4}}><p style={{fontFamily:"var(--font-display)",fontSize:16,fontWeight:800,color:C.textPri,letterSpacing:"-.01em",marginBottom:4}}>{adv.name}</p>
          <div style={{display:"inline-flex",background:`${adv.accent}18`,border:`1px solid ${adv.accent}44`,borderRadius:16,padding:"3px 12px",marginBottom:12}}>
            <span style={{fontFamily:"var(--font-mono)",fontSize:9,color:adv.accent,letterSpacing:".16em",textTransform:"uppercase"}}>{adv.title}</span>
          </div>
          <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
            {adv.expertise.map(tag=>(<span key={tag} style={{fontFamily:"var(--font-mono)",fontSize:8.5,color:C.textDim,background:C.accentG3,border:`1px solid ${C.border}`,borderRadius:8,padding:"2px 9px",letterSpacing:".1em",textTransform:"uppercase"}}>{tag}</span>))}
          </div>
        </div>
      </div>
      <div style={{height:1,background:`linear-gradient(90deg,transparent,${C.border},transparent)`,margin:"20px 28px"}}/>
      <div style={{padding:"0 28px"}}><p style={{fontFamily:"var(--font-sans)",fontSize:13,color:C.textSec,lineHeight:1.78,margin:0}}>{adv.desc}</p></div>
      <div style={{display:"flex",gap:0,marginTop:24,borderTop:`1px solid ${C.border}`}}>
        {adv.impact.map((s,i)=>(<div key={s.lbl} style={{flex:1,textAlign:"center",padding:"16px 12px",borderRight:i<adv.impact.length-1?`1px solid ${C.border}`:"none"}}><p style={{fontFamily:"var(--font-display)",fontSize:22,fontWeight:800,color:hov?adv.accent:C.textPri,margin:0,transition:"color .25s"}}>{s.val}</p><p style={{fontFamily:"var(--font-mono)",fontSize:8.5,color:C.textDim,letterSpacing:".15em",textTransform:"uppercase",margin:0}}>{s.lbl}</p></div>))}
        <div style={{flex:2,display:"flex",alignItems:"center",justifyContent:"center",padding:"16px 16px"}}><span style={{fontFamily:"var(--font-mono)",fontSize:8.5,color:C.textDim,letterSpacing:".12em",textTransform:"uppercase",textAlign:"center"}}>Academic Advisor<br/>SCAFIS Advisory Board</span></div>
      </div>
    </M.div>
  );
}
...
function AdvisorsSection() {
  return (
    <section style={{background:C.bg,padding:"96px 24px",borderTop:`1px solid ${C.border}`}}>
      <div style={{maxWidth:1080,margin:"0 auto"}}>
        <FV><Tag label="Advisory Board"/><SectionTitle>Mentors &amp; Academic Advisors</SectionTitle>
          <p style={{fontFamily:"var(--font-sans)",fontSize:14,color:C.textSec,margin:"10px 0 52px",maxWidth:500,lineHeight:1.75}}>
            Distinguished academics and industry leaders who shape SCAFIS's intellectual and strategic foundation.
          </p>
        </FV>
        <div style={{position:"relative"}}>
          <div style={{display:"flex",flexWrap:"wrap",gap:20}}>{ADVISORS.map((adv,i)=>(<AdvisorCard key={adv.initials} adv={adv} idx={i}/>))}</div>
          <FV style={{marginTop:20,textAlign:"center"}}>
            <div style={{display:"inline-flex",alignItems:"center",gap:12}}><div style={{width:32,height:1,background:C.border}}/><span style={{fontFamily:"var(--font-mono)",fontSize:8.5,color:C.textDim,letterSpacing:".2em",textTransform:"uppercase"}}>Connection to Centre Links</span><div style={{width:32,height:1,background:C.border}}/></div>
          </FV>
        </div>
      </div>
    </section>
...
/* ── Team Section (Cards) ───────────────────────────────────────────────────── */
const TEAM_MEMBERS = [
  { initials: "FA", name: "Farhan", title: "Legal Advisor", accent: C.green, desc: "Handles legal framework, contracts, and corporate compliance for the SCAFIS platform." },
...
function TeamSection() {
  return (
    <section style={{background:C.bg,padding:"64px 24px 96px",borderTop:`1px solid ${C.border}`}}>
      <div style={{maxWidth:1080,margin:"0 auto"}}>
        <FV><Tag label="Team"/><SectionTitle>The People of SCAFIS</SectionTitle></FV>
        <div style={{height:44}}/>
        <M.div initial="hidden" whileInView="show" viewport={{once:true,amount:.05}} variants={stagger}
          style={{display:"flex",flexWrap:"wrap",gap:16}}>
          {TEAM_MEMBERS.map(m => (
            <M.div key={m.initials} variants={up} style={{flex:"1 1 280px",maxWidth:340,background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"24px",display:"flex",gap:16,alignItems:"flex-start"}}>
              <Avatar initials={m.initials} size={50} accent={m.accent}/>
              <div>
                <p style={{fontFamily:"var(--font-display)",fontSize:15,fontWeight:700,color:C.textPri,marginBottom:2}}>{m.name}</p>
                <p style={{fontFamily:"var(--font-mono)",fontSize:9,color:m.accent,letterSpacing:".12em",textTransform:"uppercase",marginBottom:8}}>{m.title}</p>
                <p style={{fontFamily:"var(--font-sans)",fontSize:12.5,color:C.textSec,lineHeight:1.7,margin:0}}>{m.desc}</p>
              </div>
            </M.div>
          ))}
        </M.div>
...
/* ── Team Marquee (Finale) ───────────────────────────────────────────────────── */
const MARQUEE_DATA = [
  {initials:"RZ",name:"Rana Zain",role:"Chairman",dept:"Founder",accent:C.accent},{initials:"AK",name:"Ayesha Kashif",role:"CEO",dept:"Executive",accent:"#a78bfa"},
  {initials:"MS",name:"Prof. Saad",role:"Tax Advisor",dept:"Advisory",accent:"#22d3ee"},{initials:"MR",name:"Prof. Rehan",role:"Compliance",dept:"Advisory",accent:"#f59e0b"},
  {initials:"FA",name:"Farhan",role:"Legal",dept:"Team",accent:C.green},{initials:"HT",name:"Team Member",role:"Engineering",dept:"Team",accent:C.green},
  {initials:"SM",name:"Team Member",role:"Engineering",dept:"Team",accent:C.green},{initials:"UA",name:"Team Member",role:"QA",dept:"Team",accent:C.green},
];
...
function MqCard({ p }) {
  ...
  return (
    <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{flexShrink:0,width:176,display:"flex",flexDirection:"column",alignItems:"center",gap:12,padding:"22px 18px",background:hov?`linear-gradient(160deg,#161b34,${C.cardDeep})`:C.card,border:`1px solid ${hov?p.accent+"55":C.border}`,borderRadius:16,margin:"0 9px",cursor:"default",transition:"all .25s",transform:hov?"scale(1.06)":"scale(1)"}}>
      <div style={{width:66,height:66,borderRadius:"50%",background:`radial-gradient(circle at 36% 36%,#252d52,${C.cardDeep})`,border:`2px solid ${hov?p.accent:C.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"var(--font-display)",fontWeight:800,fontSize:17,color:hov?p.accent:C.textSec}}>
        {p.initials}
      </div>
      <div style={{textAlign:"center"}}>
        <p style={{fontFamily:"var(--font-sans)",fontSize:12.5,fontWeight:600,color:hov?C.textPri:C.textSec,margin:0}}>{p.name}</p>
        <p style={{fontFamily:"var(--font-mono)",fontSize:9,color:hov?p.accent:C.textDim,letterSpacing:".1em",textTransform:"uppercase",margin:0}}>{p.role}</p>
      </div>
    </div>
  );
}
...
function TeamMarquee() {
  ...
  return (
    <section style={{background:C.cardDeep,padding:"80px 0",borderTop:`1px solid ${C.border}`,overflow:"hidden"}}>
      <div style={{position:"relative"}}>
        <div style={{position:"absolute",left:0,top:0,bottom:0,width:110,zIndex:2,background:`linear-gradient(90deg,${C.cardDeep},transparent)`,pointerEvents:"none"}}/>
        <div style={{position:"absolute",right:0,top:0,bottom:0,width:110,zIndex:2,background:`linear-gradient(-90deg,${C.cardDeep},transparent)`,pointerEvents:"none"}}/>
...
export default function LeadershipPage() {
  return (
    <M.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} transition={{duration:.42}} style={{background:C.bg,minHeight:"100vh"}}>
      <Navbar/>
      <Hero/>
      <HierarchyIntro/>
      <FounderSpotlight/>
