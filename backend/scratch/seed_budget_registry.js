...
async function seed() {
  console.log('=== SEEDING FP&A BUDGET DATA FOR ALL COMPANIES ===');

  try {
    const companies = await db('companies').select('id');
    console.log(`Discovered ${companies.length} companies to seed.`);

    for (const c of companies) {
      const companyId = c.id;
      console.log(`\nSeeding Company ID: ${companyId}...`);

      // Find valid postable accounts in Chart of Accounts
      const accounts = await db('accounts').where({ company_id: companyId });
      if (accounts.length < 3) {
        console.log(`⚠️ Need at least 3 accounts in database for Company ${companyId}. Seeding default accounts...`);
        // Insert basic accounts if missing
        await db('accounts').insert([
          { company_id: companyId, code: '1000', name: 'Marketing Expense', is_postable: true, category: 'Expense' },
          { company_id: companyId, code: '2000', name: 'Operational Expense', is_postable: true, category: 'Expense' },
          { company_id: companyId, code: '3000', name: 'Human Resource Expense', is_postable: true, category: 'Expense' }
      ...
      // Delete old budgets for this company to seed cleanly
      await db('budget_headers').where({ company_id: companyId, fiscal_year: '2026' }).delete();

      // 1. Create Active Budget Header
      ...
      // 2. Create Budget Control Lines
      const [lineId1Obj] = await db('budget_control_lines').insert({
        budget_header_id: headerId,
        account_id: acc1.id,
        department: 'Marketing',
        branch: 'Main Head Office',
        allocated_amount: 120000.00,
        current_budget_amount: 120000.00,
        alert_threshold_pct: 90.00,
        control_level: 'BLOCK'
      }).returning('id');
      const lineId1 = typeof lineId1Obj === 'object' ? lineId1Obj.id : lineId1Obj;

      const [lineId2Obj] = await db('budget_control_lines').insert({
        budget_header_id: headerId,
        account_id: acc2.id,
        department: 'Operations',
        branch: 'Main Head Office',
        allocated_amount: 240000.00,
        current_budget_amount: 240000.00,
        alert_threshold_pct: 85.00,
        control_level: 'WARNING'
      }).returning('id');
      const lineId2 = typeof lineId2Obj === 'object' ? lineId2Obj.id : lineId2Obj;

      const [lineId3Obj] = await db('budget_control_lines').insert({
        budget_header_id: headerId,
        account_id: acc3.id,
        department: 'HR',
        branch: 'Main Head Office',
        allocated_amount: 180000.00,
        current_budget_amount: 180000.00,
        alert_threshold_pct: 95.00,
        control_level: 'NONE'
      }).returning('id');
      const lineId3 = typeof lineId3Obj === 'object' ? lineId3Obj.id : lineId3Obj;

      console.log('Created 3 budget control lines.');

      // 3. Create Monthly splits (12 months split equally)
      const months = Array.from({ length: 12 }, (_, i) => i + 1);
      for (const m of months) {
        await db('budget_monthly_allocations').insert([
          { budget_control_line_id: lineId1, month: m, allocated_amount: 10000.00 },
          { budget_control_line_id: lineId2, month: m, allocated_amount: 20000.00 },
          { budget_control_line_id: lineId3, month: m, allocated_amount: 15000.00 }
        ]);
      }
    ...
    console.log('\n✅ DATABASE SEEDED SUCCESSFULLY FOR ALL COMPANIES.');
    process.exit(0);
  } catch (err) {
